...
## NOTE: change this to eval=FALSE if pandoc is not installed
## You will not be able to render a table of contents in the .md file, though. 

pandoc --template=toc-template.txt --toc -t html homework.md --highlight-style tango |\
...
What we've done here is only look at the genes with variable expression levels, as these tend to be the most interesting for looking at differential expression. Prior to this filtering step, we had `r dim(pbmc@raw.data)[1]` genes. Seurat does this for us, so let's not worry about it. 
